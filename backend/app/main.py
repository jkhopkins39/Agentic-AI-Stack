...
# Create FastAPI app
app = FastAPI(title="Agentic AI Stack API", version="1.0.0")
...
@app.get("/")
async def root():
    return {"message": "Agentic AI Stack API is running"}
...
async def chat(request: ChatRequest):
    try:
        ...
        # Initialize state for the agent graph
        state = {
        ...
        response_content = "I'm here to help you. How can I assist you today?"
        ...
        try:
            save_query_to_conversation(
                ...
                response_content,
                1,
                result.get("conversation_context", {}).get("user_id")
            )
...
async def get_conversation(session_id: str):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT q.query_text, q.agent_response, q.created_at, q.agent_type
            FROM queries q
            JOIN conversations c ON q.conversation_id = c.id
            WHERE c.session_id = %s
            ORDER BY q.message_order, q.created_at
        """, (session_id,))
...
@app.post("/conversations/{session_id}/reset")
async def reset_conversation(session_id: str):
    ...
    try:
        ...
        cursor = conn.cursor()
        # Mark conversation as completed and create a new one
        cursor.execute("""
        ...
        new_conversation_id = create_conversation(session_id)
        conn.commit()
        cursor.close()
        conn.close()
        
        return {"message": "Conversation reset successfully", "new_conversation_id": new_conversation_id}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def query_policy(q: str):
    ...
    try:
        formatted_response, response_text = query_rag(q)
        return {"query": q, "response": response_text, "formatted_response": formatted_response}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def router(state: State):
    ...
    # Return the state with routing information
    if message_type == "Order":
        return {"next": "order", "message_type": message_type}
    if message_type == "Email":
        return {"next": "email", "message_type": message_type}
    if message_type == "Policy":
        return {"next": "policy", "message_type": message_type}
    if message_type == "Change Information":
        return {"next": "message", "message_type": message_type}
...
def get_database_connection():
    try:
        # Use DATABASE_URL from environment if available, otherwise fall back to individual variables
